    ...
    switch(srcName) {
		case "SEARCHLIST":
			...
			if(empCond==2){
				...
				formObj.action = "./AIE_FRT_0020.clt";
				formObj.submit();				
			}
		break;
		case "SEARCHLIST02":	//House B/L 목록을 조회함
			if(frm1.intg_bl_seq.value!=''){
			    ...
			    docObjects[0].DoSearch4Post("AIE_FRT_0020_1GS.clt", FormQueryString(formObj));
			}
		break;
		case "SEARCHLIST03":	//Buying/Credit Tariff 목록을 조회함
			if(frm1.intg_bl_seq.value!=''){	
	       		formObj.f_cmd.value = SEARCHLIST03; 
			    docObjects[1].DoSearch4Post("AIE_FRT_0020_2GS.clt", FormQueryString(formObj));
			}
		break;
		case "ROWADD":
			if(frm1.intg_bl_seq.value==''){
		...
		case "SAVE":	//등록/수정/삭제시
			...
			if(bcFlg==true){
				...
				if(confirm(getLabel('FMS_COM_CFMCON'))){
					docObjects[1].DoAllSave("./AIE_FRT_0020_2GS.clt", FormQueryString(formObj)+'&'+docObjects[0].GetSaveString(false), true);
				}
			}
		break;
		case "HBL_POPLIST":
