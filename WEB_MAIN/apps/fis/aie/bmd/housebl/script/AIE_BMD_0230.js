	...
	try {
		...
		case "MODIFY":
			...
			if (confirm(getLabel('FMS_COM_CFMSAV'))) {
				...
				formObj.action = "./AIE_BMD_0230.clt";
				// formObj.submit();
				submitForm(MODIFY);
			}
...
//			}
		...
		case "ROWADD2":
			...
			sheetObj.SetCellValue(intRows-1,"pck_dt",todayStr().replaceAll("-","") )
			break;
		case "SAVE2":
         ...
         case 1:      //IBSheet1 init
            with (sheetObj) {
             ...
             var cols = [ {Type:"DelCheck", Hidden:0, Width:50,   Align:"Center",  ColMerge:0,   SaveName:"",             KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:2 },
                          ...
                          {Type:"Date",      Hidden:0,  Width:80,   Align:"Right",    ColMerge:0,   SaveName:"pck_dt",       KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
                          ...
                          {Type:"Float",      Hidden:0,  Width:80,   Align:"Right",    ColMerge:0,   SaveName:"item_ru",       KeyField:0,   CalcLogic:"",   Format:"Float",            PointCount:2,   UpdateEdit:1,   InsertEdit:1 },
...
function setMrdFile(){
    ...
    if (formObj.print_yn.value == "Y" && formObj.f_wrk_tp.value != "") {
		formObj.file_name.value = 'packing_list_01_BNXC.mrd';
...
function MBL_POPLIST(rtnVal){
      ...
      if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
		...
		formObj.txt_hbl_no.value=rtnValAry[0];//wo_no
		formObj.hbl_no.value=rtnValAry[0];//wo_no
...
function sheet1_OnChange(sheetObj, Row, Col, Value){
	...
	if(Col == 1){
		$.each(itemData.filter(function(value){return (value.index == Value)}), function(idx, item){
			//#3636 [BINEX] AEM C/I for Produce Cargo QTY ë¡œì§� ë³€ê²½ í•„ìš”
			...
			sheetObj.SetCellValue(Row, "inv_amt",Number(item.inv_amt) * item_qty, 0);
