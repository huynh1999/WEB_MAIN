		...
		switch(srcName) {
			case "SEARCHLIST":
	    	    if(formObj.txt_hbl_no.value == ""){
	       		...
	       		//formObj.action="./AIE_BMD_0140.clt";
			    //formObj.submit();
 ...
 */
function loadPage() {
	//#1170 [Contract] Cannot auto suggestion when name inputted
	fnSetAutocompleteCallBack('f_carr_trdp_nm', 'LINER_POPLIST', 'LINER_POPLIST'); //Customer	
	var formObj=document.frm1;
...
function fn_ComGetLenByByte(obj, len)
{
    try {
        ...
        for (var inx = 0; inx < sValue.length; inx++) {
            ...
            else if (oneChar.indexOf("%0A")!= -1)   { byteLength += 2;} //줄바꿈
            else if (oneChar.indexOf("%") != -1)    { byteLength += oneChar.length/3; }
            
            //console.log(byteLength + '  / ' + sValue.charAt(inx) + '  /  '+ oneChar + ' /  ' + oneChar.length);
            if(len <= byteLength ){
            	return obj.value = sValue.substring(0, inx-1);
            }
        } // end for
        //console.log("byteLength  = "  + byteLength);
        return obj.value = sValue;
