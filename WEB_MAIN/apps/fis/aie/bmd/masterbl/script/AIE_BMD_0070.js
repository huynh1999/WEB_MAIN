		...
		switch(srcName) {
	   	 ...
	   	 case 'TSA':
		   	 	if(sheetObj1.RowCount() == 0){
		   	 		...
		   	 		popGET('AIE_BMD_0200.clt'+reqParam, '', 600, 280, "scroll:yes;status:no;help:no;");
		   	 	}
...
//		   	 			alert("Only KE and OZ can access to Cargo Tracking");
	   	 	...
	   	 	break;
	   	 	case "GOTO_KMS": //#5167 [BINEX] KAMS BUTTON ADD ON Air Export > Master AWB > AEM AWB List
		   	 	var blNo=sheetObj1.GetCellValue(sheetObj1.GetSelectRow(), "bl_no");
				var refOfcCd=sheetObj1.GetCellValue(sheetObj1.GetSelectRow(), "ref_ofc_cd");
		   	 	if(sheetObj1.RowCount()== 0){
		   	 		//There is no data
	   	 			alert(getLabel('FMS_COM_ALT004'));
		   	 	}else{
		   	 		var param='&f_bl_no=' + blNo + '&f_ofc_cd=' + refOfcCd;
		   	 		
		   	 		var paramStr="./EDI_AMS_0010.clt?f_cmd="+param;
		   	 		parent.mkNewFrame('AE KMS EDI', paramStr);
				}
	   	 	break;
	   	 	case "DOCK_RECEIPT":
				if(sheetObj1.RowCount()== 0){
			...
			break;		   	 	
   	 		...
   	 		case "PRINT":
					...
					param += '&house_bl_no=' + docObjects[0].GetCellValue(docObjects[0].GetSelectRow(), "bl_no");
   	 				...
   	 				param += '&by_carr=' + docObjects[0].GetCellValue(docObjects[0].GetSelectRow(), "lnr_trdp_nm");
...
//   	 				}
					...
					param += '&mailTitle=' + 'Master Air Way [BL No : ' + docObjects[0].GetCellValue(docObjects[0].GetSelectRow(), "bl_no") + ']';
   	 				...
   	 				popGET('RPT_PRN_0080.clt?'+param, '', 420, 690, "scroll:yes;status:no;help:no;");
   	 			}
   	 			break;
   	 		case "COPY":
...
//	   	 		}
	   	...
	   	//#27548 [BINEX]BL 공통-HC 단축키 추가
			...
			var formObj=document.frm1;
			formObj.file_name.value = 'air_mbl_label_01_UFF.mrd';
			formObj.title.value='Small Label';
			// Parameter Setting
		...
		case "FILE_LABEL":
			...
			case "EXCEL_ALL":
				// Check if don't have data on grid, cannot download excel file.
				if(docObjects[0].RowCount() < 1){ //no data	
...
//	}
	...
	ajaxSendPost(setOrderByInfo, 'reqVal','&goWhere=aj&bcKey=searchTbUserOrderbyInfoAttr&pgm_usr_id='+formObj.user_id.value+'&pgm_url=./AIE_BMD_0070.clt',	'./GateServlet.gsl');
		 ...
		 case 2:      //IBSheet1 init
		    with (sheetObj) {
	            ...
	            var cols = [ {Type:"Seq",      Hidden:0,  Width:40,   Align:"Center",  ColMerge:0,   SaveName:"no",            KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1 },
	                ...
	                {Type:"Text",      Hidden:1, Width:70,   Align:"Right",   ColMerge:0,   SaveName:"hbl_intg_bl_seq",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 }];
	            InitColumns(cols);
	            SetEditable(1);
...
//		#549 [SBS] BL List 에서 Entry 조회시 B/L SEQ 로 조회 하도록 수정
	   	var paramStr="./AIE_BMD_0020.clt?f_cmd="+SEARCHLIST+"&f_bl_no="+escape(hblNo)+"&f_intg_bl_seq="+encodeURIComponent(hblSeq);
...
function excelDown(mySheet){
	...
	// Don't allow downloading excel file if From Date is blank.
