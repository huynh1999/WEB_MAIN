...
function doWork(srcName, curObj, valObj){
	...
	switch(srcName) {
		case "SEARCH":
			if(formObj.cust_ord_no.value==''){
				//alert('[B/L No.]를 먼저 입력하십시오!'); 
				alert(getLabel('FMS_COM_ALT001')+ "\n\n: CMM_POP_0320.9");
				formObj.cust_ord_no.focus();
			}else{
	       		formObj.f_cmd.value=SEARCH;
	   			formObj.action="./CMM_POP_1000.clt";
			    formObj.submit();
			}
		break;
        case "PRINT":
        	...
        	formObj.file_name.value=formObj.mrd_file_nm.value;
        	formObj.title.value='Delivery Order';
			//Parameter Setting
			var param='';
			param += '[' + formObj.wob_bk_no.value + ']'; // $1
			param += '[' + formObj.ofc_eng_nm.value + ']';// $2
			param += '[' + formObj.eml.value + ']';// $3
			param += '[' + formObj.user_name.value + ']';// $4
			param += '[' + formObj.trsp_trdp_nm.value + ']';//$5
			param += '[' + formObj.dest_rout_trdp_cd.value + ']'; //$6
			param += '[' + formObj.dest_rout_trdp_nm.value + ']'; //$7
			param += '[' + formObj.dest_rout_addr.value + ']'; // $8
			param += '[' + formObj.dest_rout_pic.value + ']';// $9
			param += '[' + formObj.dest_rout_ref.value.toUpperCase() + ']';//$10
			param += '[' + formObj.dest_rout_date.value.toUpperCase() + ']';//$11
			param += '[' + formObj.dest_rout_trdp_cd.value + ']'; //$12
			param += '[' + formObj.dest_rout_trdp_nm.value + ']'; //$13
			param += '[' + formObj.bill_to_trdp_addr.value + ']'; // $14
			param += '[' + formObj.special_ins.value + ']';// $15
			param += '[' + formObj.do_rmk.value + ']'; // $16
			param += '[' + formObj.pickup_trdp_cd.value + ']'; //$17			
			param += '[' + formObj.pickup_trdp_nm.value + ']'; //$18
			param += '[' + formObj.pickup_addr.value + ']'; // $19
			param += '[' + formObj.pickup_pic.value + ']';// $20
			param += '[' + formObj.pickup_ref.value.toUpperCase() + ']';//$21
			param += '[' + formObj.pickup_date.value.toUpperCase() + ']';//$22
			param += '[' + formObj.trsp_trdp_cd.value + ']'//$23
			param += '[' + formObj.trsp_trdp_addr.value + ']'//$24
			param += '[' + formObj.tel.value + ']';// $25
			param += '[' + formObj.fax.value + ']';// $26
			param += '[' + formObj.ofc_cd.value + ']';// $27
			formObj.rd_param.value=param;

			formObj.rpt_trdp_cd.value=formObj.trsp_trdp_cd.value;
			formObj.mailTitle.value='Delivery Order [HBL No : ' + formObj.cust_ord_no.value + ']'; //[20140112 OJG] 이메일 제목추가
			
			//#52512 [CLT] RD File Name을 표준화| Standardization of File Name during downloading the report 
			if (formObj.cust_ord_no.value != ""){
				var v_bl_no = formObj.cust_ord_no.value ;
				v_bl_no = v_bl_no.replace(/\./g, "");
				v_bl_no = v_bl_no.replace(/\\|\/|\:|\*|\?|\"|\<|\>|\||\&|\-|\__|\s/g, "_");
				formObj.rpt_file_name_title.value = "Delivery_Order _"+v_bl_no ;
			} else {
...
//			formObj.dest_rout_pic_1.value=rtnValAry[3];//pic_nm
//			formObj.dest_rout_pic_phn_1.value=rtnValAry[4];//pic_phn
			//formObj.dest_rout_pic_fax.value=rtnValAry[5];//pic_fax
//		} else if(id=="del_2") {
//			var rtnValAry=rtnVal.split("|");
//			formObj.dest_rout_trdp_cd_2.value=rtnValAry[0];//trdp_cd
//			formObj.dest_rout_trdp_nm_2.value=rtnValAry[2];//eng_nm
//			//formObj.dest_rout_addr.value=rtnValAry[7];//eng_addr
//			formObj.dest_rout_addr_2.value=rtnValAry[37];//dflt_addr
//			//formObj.dest_rout_pic_fax.value=rtnValAry[5];//pic_fax
		}else if(id=="pck") {
			console.log(rtnValAry)
			var rtnValAry=rtnVal.split("|");
...
function loadPage() {
	 ...
	 // codeNameAction1('trdpCode_del',formObj.s_dest_rout_trdp_cd, 'onBlur');
		fnSetAutocomplete('trsp_trdp_nm'		, 'LINER_POPLIST', 'trk', 'O'); 		//Trucker
}
...
//			formObj.pickup_pic_phn.value="";//pic_phn
		...
		else if(str =="trdpCode_bil"){
			formObj.bill_to_trdp_cd.value="";//trdp_cd  AS param1
			formObj.bill_to_trdp_nm.value="";//eng_nm   AS param2
			formObj.bill_to_trdp_addr.value="";//dflt_addr
...
// 				formObj.trsp_trdp_nm.value=masterVals[16];//locl_nm   
 				...
 				//formObj.dest_rout_addr.value=masterVals[1];//dflt_addr
...
// 				formObj.dest_rout_pic_fax.value=masterVals[12];//pic_fax	
 			}else if(CODETYPE =="trdpCode_bil"){
 				formObj.bill_to_trdp_cd.value=masterVals[0];//trdp_cd  AS param1
 				formObj.bill_to_trdp_nm.value=masterVals[3];//eng_nm   AS param2
 				//formObj.dest_rout_addr.value=masterVals[1];//dflt_addr
 				formObj.bill_to_trdp_addr.value=masterVals[1];//eng_addr
// 				formObj.bill_to_pic.value=masterVals[10];//pic_nm
// 				formObj.dest_rout_pic_phn.value=masterVals[11];//pic_phn
// 				formObj.dest_rout_pic_fax.value=masterVals[12];//pic_fax	
 			}else if(CODETYPE =="trdpCode_pickup"){
 				console.log("trdpCode_pickup");
...
//				formObj.dest_rout_pic_fax.value="";//pic_fax	
 				...
 				formObj.bill_to_trdp_nm.value="";//eng_nm
 				formObj.bill_to_trdp_addr.value="";//eng_addr
// 				formObj.bill_to_trdp_addr.value=rtnValAry[37];//dflt_addr
 			}else if(CODETYPE =="trdpCode_pickup"){
 				formObj.pickup_trdp_cd.value="";//trdp_cd  AS param1
 				formObj.pickup_trdp_nm.value="";//eng_nm   AS param2
