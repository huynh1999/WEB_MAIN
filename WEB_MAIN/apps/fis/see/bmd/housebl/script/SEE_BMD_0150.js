...
//	       		formObj.action="./SEE_BMD_0150.clt";
//			    formObj.submit();
	    	    formObj.f_cmd.value=SEARCHLIST;
...
function loadPage() {
	...
	if(formObj.print_yn.value == "Y" && formObj.f_wrk_tp.value != ""){
		formObj.file_name.value='certificateoforigin_01.mrd';
    	formObj.title.value='CERTIFICATE OF ORIGIN';
		//Parameter Setting
    	var param='[' + formObj.intg_bl_seq.value + ']';			// [1]
    	param 	  += '[' + formObj.f_wrk_tp.value + ']';			// [2]
		formObj.rd_param.value=param;
   	 	formObj.rpt_biz_tp.value="SEE_BMD_0150";
		formObj.rpt_biz_sub_tp.value="";
		popPOST(frm1, 'RPT_RD_0010.clt', 'popTest', 1025, 740);
		//alert(getLabel('FMS_COM_CFMSAV'));
...
function submitForm(){
	...
	$.ajax({
		   ...
		   success: function(data){
			   ...
			   setFieldValue( formObj.f_agent, $('agent_addr',data).text());
			   setFieldValue( formObj.f_bl_no, $('doc_recpt_no',data).text());
			   ...
			   setFieldValue( formObj.f_point_entry, $('point_entry',data).text());
