...
var dimListSheet=false;		//BP #1762 [IMPEX] OCEAN QUOTATION TO HAVE DIMENSION BOX
...
function getSndParam(){
	...
	var dimListParam=docObjects[3].GetSaveString(false);	//BP #1762 [IMPEX] OCEAN QUOTATION TO HAVE DIMENSION BOX
	...
	//BP #1762 [IMPEX] OCEAN QUOTATION TO HAVE DIMENSION BOX
	if(dimListParam!=''){
	  	sheetParam+= '&';
...
/*			    [OFVFOUR-7716] [PQC][OE Quotation Entry] The system load missing data for Carrier and Mode/TT
        	...
        	case "SAVE_MODIFY":	//수정
    			...
    			if(qttnCheckInpuVals()){
        		   ...
        		   if(confirm(getLabel('FMS_COM_CFMSAV'))){
	            	   ...
	            	   var sndParam=getSndParam();
	        		   if(sndParam == true)	{	return false;	}	 
	            	   doShowProcess(); console.log('\n['+FormQueryString(frm1)+sndParam+']\n');
	            	   docObjects[0].DoAllSave("./SEE_BMD_0300GS.clt", FormQueryString(frm1)+sndParam, false);
        		   }
    			}
    			break;
           ...
           case "SEARCH_CHG":	// Charge 조회
        	   ...
        	   //임시
        	   //frm1.qttn_seq.value = frm1.f_qttn_seq.value;
           ...
           case "SEARCH_DIM":	// Dimension 조회
        	   ...
        	   //임시
        	   //frm1.qttn_seq.value = frm1.f_qttn_seq.value;
        	   docObjects[3].DoSearch("./SEE_BMD_0300_3GS.clt", FormQueryString(frm1) );
        	   break;
...
function sheet1_OnSaveEnd(sheetObj, errMsg){
	...
	dimListSheet=false; //BP #1762 [IMPEX] OCEAN QUOTATION TO HAVE DIMENSION BOX
...
function loadPage() {
	...
	fnSetAutocompleteCallBack('por_nm', 'POR_LOCATION_POPLIST', 'LOCATION_POPLIST', 'O');
	fnSetAutocompleteCallBack('pol_nm', 'POL_LOCATION_POPLIST', 'LOCATION_POPLIST', 'O');
	fnSetAutocompleteCallBack('pod_nm', 'POD_LOCATION_POPLIST', 'LOCATION_POPLIST', 'O');
	fnSetAutocompleteCallBack('del_nm', 'DEL_LOCATION_POPLIST', 'LOCATION_POPLIST', 'O');
	fnSetAutocompleteCallBack('fnl_dest_loc_nm', 'DEST_LOCATION_POPLIST', 'LOCATION_POPLIST', 'O');
    ...
    switch(sheetNo) {
		...
		case 4:     //Calculate CBM
			with(sheetObj){
		        ...
		        var info    = { Sort:1, ColMove:1, HeaderCheck:1, ColResize:1 };
		        var headers = [ { Text:getLabel('SEE_BMD_0300_HDR3'), Align:"Center"} ];
		        InitHeaders(headers, info);
...
function weightChange(obj){
	...
	if(obj.name=="grs_wgt_kg"){
		var rndXLValue=roundXL(formObj.grs_wgt_kg.value.replaceAll(",","") * CNVT_CNST_KG_LB, 2);
		formObj.grs_wgt_lbs.value=rndXLValue;
		chkComma(formObj.grs_wgt_lbs, 9, 2);
	}
	else if(obj.name=="grs_wgt_lbs"){
		var rndXLValue=roundXL(formObj.grs_wgt_lbs.value.replaceAll(",","") / CNVT_CNST_KG_LB, 2);
...
function loadData(){
	...
	doWork('SEARCH_CHG');
	doWork('SEARCH_OTH_CHG');
	doWork('SEARCH_DIM');		//BP #1762 [IMPEX] OCEAN QUOTATION TO HAVE DIMENSION BOX
}
...
//Begin BP #1762 [IMPEX] OCEAN QUOTATION TO HAVE DIMENSION BOX
...
*/
//End BP #1762 [IMPEX] OCEAN QUOTATION TO HAVE DIMENSION BOX