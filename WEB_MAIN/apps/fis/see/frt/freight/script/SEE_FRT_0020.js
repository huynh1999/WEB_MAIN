...
function doWork(srcName){
    ...
    switch(srcName) {
		case "SEARCHLIST":
			...
			formObj.f_sr_no.value = trim(formObj.f_sr_no.value);
			if(formObj.f_sr_no.value==''){
				empCond++;
			}
			formObj.f_master_bl_no.value = trim(formObj.f_master_bl_no.value);
			...
			if(empCond==3){
				...
				formObj.action = "./SEE_FRT_0020.clt";
				formObj.submit();				
			}
		...
		case "SEARCHLIST01":	//TP/SZ 목록을 조회함
			...
			if(formObj.intg_bl_seq.value!=''){
			    ...
			    sheetObj1.DoSearch4Post("SEE_FRT_0020GS.clt", FormQueryString(formObj));
			}
		...
		case "SEARCHLIST02":	//House B/L 목록을 조회함
		    ...
		    sheetObj2.DoSearch4Post("SEE_FRT_0020_1GS.clt", FormQueryString(formObj));
		
		    break;
		
		case "SEARCHLIST03":	//Buying/Credit Tariff 목록을 조회함
       		formObj.f_cmd.value = SEARCHLIST03; 
		    sheetObj3.DoSearch4Post("SEE_FRT_0020_2GS.clt", FormQueryString(formObj));
		...
		case "SAVE":	//등록/수정/삭제시
			...
			if(bcFlg==true){
				...
				if(confirm(getLabel('FMS_COM_CFMCON'))){
					sheetObj3.DoAllSave("./SEE_FRT_0020_2GS.clt", FormQueryString(formObj)+'&'+sheetObj2.GetSaveString(false), false);
				}
			}
...
//Buy/Credit 승인시 기능 버튼 Disable
function setBuyCreditCnf(reqVal){
    var doc = getAjaxMsgXML(reqVal);
    if(doc[0]=='OK'){
    	var obj = getObj('bcBtns');
    	obj.style.display = 'none';
    	
    	getObj('bcUnCnfTb').style.display = 'block';
    	
    	if(!dispBc){
    		getObj("saveBtn").style.display = 'none';
    	}
...
//Buy/Credit 승인시 기능 버튼 Enable
function setBuyCreditCnfRbk(reqVal){
    var doc = getAjaxMsgXML(reqVal);
    if(doc[0]=='OK'){
    	var obj = getObj('bcBtns');
        obj.style.display = 'block';
    	
        //Confirm Cancel 버튼 처리
    	getObj('bcUnCnfTb').style.display = 'none';

    	//Save 버튼 활성화
    	getObj('saveBtn').style.display = 'block';

    	//Grid 수정/삭제 처리
    	docObjects[2].Editable = true;
