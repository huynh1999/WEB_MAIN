    ...
    switch(srcName) {
		case "SEARCHLIST":
			...
			if(empCond==2){
				...
				formObj.action = "./AII_FRT_0010.clt";
				formObj.submit();				
			}
		break;
		case "SEARCHLIST02":	//Selling/Debit Tariff 목록을 조회함
			if(frm1.intg_bl_seq.value!=''){
			    ...
			    sheetObj1.DoSearch4Post("AII_FRT_0010_1GS.clt", FormQueryString(formObj));
			}
		break;
		case "SEARCHLIST03":	//Buying/Credit Tariff 목록을 조회함
			if(frm1.intg_bl_seq.value!=''){
	       		formObj.f_cmd.value = SEARCHLIST03; 
			    sheetObj4.DoSearch4Post("AII_FRT_0010_1GS.clt", FormQueryString(formObj));
			}
		break;
		case "ROWADD":
			if(frm1.intg_bl_seq.value==''){
			...
			if(sdFlg==true&&bcFlg==true){
				...
				if(confirm(getLabel('FMS_COM_CFMCON'))){
					...
					if(frm1.f_CurTab.value=='01'){
						sheetObj1.DoAllSave("./AII_FRT_0010_1GS.clt", FormQueryString(formObj)+'&'+sheetObj4.GetSaveString(false), true);
						
					}else{
						sheetObj4.DoAllSave("./AII_FRT_0010_1GS.clt", FormQueryString(formObj)+'&'+sheetObj1.GetSaveString(false), true);
					}
				}
			}
		break;
