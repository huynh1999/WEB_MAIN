...
function doWork(srcName){
	...
	switch(srcName) {
		case "SEARCHLIST01":	//OB Date, 편명목록 조회
			...
			docObjects[0].DoSearch("EDI_CSTM_0060GS.clt",   FormQueryString(frm1) );
		break;
		case "SEARCHLIST02":	//BL목록 조회
			frm1.f_cmd.value=SEARCHLIST02;
			docObjects[1].DoSearch("EDI_CSTM_0060_1GS.clt", FormQueryString(frm1) );
		break;
		case "COMMAND01":		//전송데이터 생성
			if(checkInpuVals()){
				...
				if(confirm(getLabel('FMS_COM_CFMCRE'))){
					...
					docObjects[0].DoAllSave("./EDI_CSTM_0060GS.clt?" + param, "");
				}
			}
		break;
		case "COMMAND02":		//전송데이터 수정
			if(checkInpuVals()){
				...
				if(confirm(getLabel('FMS_COM_CFMMOD'))){
					...
					docObjects[0].DoAllSave("EDI_CSTM_0060GS.clt", FormQueryString(frm1)+'&'+docObjects[1].GetSaveString(true));
				}
			}
		break;
		case "COMMAND03":		//전송데이터 삭제
			...
			if(confirm(getLabel('FMS_COM_CFMDEL'))){
				...
				docObjects[0].DoAllSave("EDI_CSTM_0060GS.clt", FormQueryString(frm1));
			}
		break;
...
function checkInpuVals(){
	...
	if(checkInputVal(frm1.disp_cstm_cd.value, 3, 3, "T", '세관코드')!='O'){
		...
		isOk=false;
/*	
	}else if(checkInputVal(frm1.noti_nm.value, 2, 100,"T", '통지처')!='O'){
		frm1.disp_mrn.focus();
		isOk=false;
*/
/*		
	...
	switch(sheetNo) {
		case 1:      //House B/L Information
			with (sheetObj) {
				...
				var headers = [ { Text:getLabel('EDI_CSTM_0060_HDR1'), Align:"Center"} ];
				...
				var cols = [ {Type:"Status",    Hidden:1, Width:0,    Align:"Left",    ColMerge:1,   SaveName:"ibflag" },
				             ...
				             {Type:"Text",      Hidden:0, Width:150,  Align:"Left",    ColMerge:1,   SaveName:"mmbl_no" },
				             {Type:"Text",      Hidden:0, Width:300,  Align:"Left",    ColMerge:1,   SaveName:"vsl_nm" },
				             {Type:"Text",      Hidden:0, Width:80,   Align:"Left",    ColMerge:1,   SaveName:"flt_no" },
				             {Type:"Combo",     Hidden:0, Width:100,  Align:"Center",  ColMerge:1,   SaveName:"edi_sts" },
				             {Type:"Text",      Hidden:0, Width:120,  Align:"Left",    ColMerge:1,   SaveName:"smt_dt" },
				             ...
				             {Type:"Text",      Hidden:1, Width:0,    Align:"Center",  ColMerge:1,   SaveName:"noti_nm" } ];
		...
		case 2:      //House B/L Information
			with (sheetObj) {
		      	SetConfig( { SearchMode:2, MergeSheet:5, Page:20, DataRowMerge:1 } );
		      	var info    = { Sort:1, ColMove:1, HeaderCheck:1, ColResize:1 };
		      	var headers = [ { Text:getLabel('EDI_CSTM_0060_HDR2_1'), Align:"Center"},
		      	                { Text:getLabel('EDI_CSTM_0060_HDR2_2'), Align:"Center"} ];
		      	InitHeaders(headers, info);
	
		      	var cols = [ {Type:"Status",   Hidden:1, Width:0,    Align:"Left",    ColMerge:1,   SaveName:"ibflag" },
				             {Type:"Text",     Hidden:1, Width:120,  Align:"Left",    ColMerge:1,   SaveName:"cur_sts" },
				             {Type:"Text",     Hidden:0, Width:100,  Align:"Left",    ColMerge:1,   SaveName:"mbl_no" },
				             {Type:"Text",     Hidden:0, Width:100,  Align:"Center",  ColMerge:1,   SaveName:"mbl_obrd_dt" },
				             {Type:"Text",     Hidden:0, Width:200,  Align:"Left",    ColMerge:1,   SaveName:"mbl_vslnm" },
				             {Type:"Text",     Hidden:0, Width:120,  Align:"Left",    ColMerge:1,   SaveName:"mbl_flt_no" },
				             {Type:"Text",     Hidden:0, Width:100,  Align:"Left",    ColMerge:1,   SaveName:"hbl_no" },
				             {Type:"Text",     Hidden:0, Width:100,  Align:"Center",  ColMerge:1,   SaveName:"hbl_obrd_dt" },
				             {Type:"Text",     Hidden:0, Width:200,  Align:"Left",    ColMerge:1,   SaveName:"hbl_vslnm" },
...
function sheet1_OnClick(sheetObj, row, col){
	...
	frm1.noti_nm.value=sheetObj.GetCellValue(row, 'noti_nm');
...
function ediValidation(sheetObj, curRow) {
	...
	for(var i=2; i<hblSheet.LastRow() + 1; i++){
		...
		var msn_no 			 = hblSheet.GetCellValue(i,"msn_no");		
		...
		var no_cntr_no_cnt	 = hblSheet.GetCellValue(i,"no_cntr_no_cnt");
		
		var row = i - 1;
		
		// Master B/L 일련 번호 Check
		if(trim(msn_no) == ""){
			valMsgArr.push("Row["+ row +"] : " +  getLabel('EDI_COM_ALT068'));
		}
		...
		// 컨테이너 번호 없는 데이터 1개 이상 Check 
		if (no_cntr_no_cnt > 0) { 
	    	valMsgArr.push("Row["+ row +"] : " +  getLabel('EDI_COM_ALT041'));
	    }
	}
