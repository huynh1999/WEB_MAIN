...
function doWork(srcName, chekVal){
    ...
    switch(srcName) {
	   ...
	   break;
   	   case "PRINT_CALLBACK":
   	   		// 25704 Print 완료 flag 비교 
   		   	isPrinted=true;
   	   		// LHK, 20140307, #26818 [BINEX]Check Print 이후 Payment 자료 수정이 되고 있음 
   	   		if (isPrinted) {
				formObj.f_cmd.value=SEARCHLIST;
				//검증로직
				docObjects[0].DoSearch("./ACC_JOR_0030GS.clt", FormQueryString(formObj) );
			}
   	   break;
   	   case "CLEAR":
...
//	   	    	alert("Please enter a Mandatory Value! (Red indicates required field) ");
    	    ...
    	    if(sheetObj.GetCellValue(1, "inv_post_dt") == ""){
	            ...
	            sheetObj.SetCellValue(intRows, "chk_flag",1);
	   	        ...
	   	        //4223 [JAPT] PEYMENT ENTRY  - can't save after rowadd
	   	        ...
	   	        sheetObj.SetCellValue(intRows, "inv_xcrt_dt", formObj.f_post_dt.value);  //#1617 [CLT] Depsosit/Payment Entry - Ex.Rate Date 정의 기준변경
	   	    }else{
	   	    	var intRows=sheetObj.LastRow() + 1;
	            sheetObj.DataInsert(intRows);
	            sheetObj.SetRowBackColor(intRows,"#EFEBEF");
    	   ...
    	   if(chekVal != 'PRNSAVE'  && !checkDelete()){
	           ...
	           if(saveCfmFlg){
	    		   ...
	    		   if(saveFlg){
			           ...
			           //sheetObj.DoAllSave("./ACC_JOR_0030GS.clt", FormQueryString(formObj)+'&'+sht2, true);
			           sheetObj.DoSave("./ACC_JOR_0030GS.clt", FormQueryString(formObj)+'&'+sht2,"ibflag", false);
	    		   }
	           }
    	   }
    	   break;
       ...
       case "CUSTOMER_POPLIST"://openMean 1=화면에서 오픈, 2=그리드에서 오픈
	   		rtnary=new Array(1);
	   		rtnary[0]="";
	   	//#779 [Payment Entry] Cannot open empty popup
...
//	   		rtnary[2]=window;
       ...
       case "CUSTOMER2_NAME":
	     ...
	     break;
        case "PRINT":
        	//LHK, 20130227 저장되지 않은 경우 save event 를 적용 후 print 한다, 
			//print 시 자동 저장 후 save end 시 'complete' message 를 보여주지 않기 위해 변수 사용
...
function INVGET(rtnVal){
    ...
    if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
		...
		//[#1203] Set wrong Vendor name and Paid to name.
		formObj.s_cust_cd.value		= rtnValAry[1];
		formObj.s_paid_cd.value  	= rtnValAry[1];
		formObj.s_cust_nm.value		= rtnValAry[2]; //Vender name
		formObj.s_paid_nm.value  	= rtnValAry[2];//Paid to name
...
function CUSTOMER_POPLIST2(rtnVal){
	...
	if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
		...
		//#3488 Accounting Multi Language 적용 
		if ( MULTI_LANGUAGE == "Y" ){		
			formObj.s_paid_nm.value=rtnValAry[10]; //locl_nm
		}else{
			formObj.s_paid_nm.value=rtnValAry[2]; //eng_nm	
		}
	}     
}
//--------------------------------------------------------------------------------------------------------------
...
function loadPage() {
    ...
    //#2487 [CLA Test] V450 Test / G&A Invoice No. Error
    formObj.s_inv_no.value = decodeURIComponent(formObj.s_inv_no.value);
}
...
function sheet1_OnSearchEnd(){
	...
	if(sheetObj.GetCellValue(1,"jnr_no") != "" ){
		...
		formObj.f_deposit_dt.value=sheetObj.GetCellValue(1, "r_clr_dt");
		formObj.f_post_dt.value=sheetObj.GetCellText(1, "r_post_dt");
		...
		for(var i=1;i<=sheetObj.LastRow();i++){
			...
			//sheetObj.SetCellValue(i, "locl_bal_amt_1", Number(sheetObj.GetCellValue(i, "locl_ttl_amt")) - Number(sheetObj.GetCellValue(i, "locl_amt")), 0);
...
//		printBtn02.style.display = "inline";
//		copyBtn01.style.display = "inline";
//		copyBtn02.style.display = "inline";
...
//	//#2979 [STAR CLUSTER] AFTER V450, PAYMENT LIST AND ENTRY HAVE DIFF POST DATE
//	if(sheetObj.GetCellValue(1,"jnr_no") != "" ){
//		//#2630 [zen] incorrect reference for block date when user void payment
//		setBlock_dt();
//	}
		...
		if(typeof(doc[1])!='undefined'){
			...
			if(CODETYPE =="CUSTOMER"){
				...
				//if(formObj.s_paid_cd.value == ""){
					...
					//#3488 Accounting Multi Language 적용 
					if ( MULTI_LANGUAGE == "Y" ){
						formObj.s_paid_nm.value=masterVals[16];		//locl_nm   AS param2				
					}else{
						formObj.s_paid_nm.value=masterVals[3];		//eng_nm   AS param2					
					}
				//}
...
function setSearchCondition(stsVal){
	...
	if(stsVal == "CLEARALL"){
		...
		if(formObj.f_jnr_no.value != ""){
			...
			formObj.s_agent_ref_no.disabled=true;
	 	}
	}
}
function clearInput(){
...
*/	
        ...
        // Date Format 맞추기 위해 sheetObj.SetCellValue => SetCellText로 변경
		...
		if(typeof(doc[1])!='undefined'){
			...
			if (rtnArr[0] == "0") {
				...
				//if (formObj.s_cust_cd.value == "" || formObj.s_cust_nm.value){
					...
					//#3488 Accounting Multi Language 적용 
					if ( MULTI_LANGUAGE == "Y" ){
						formObj.s_cust_nm.value = rtnArr[10]; //locl_nm						
					}else{
						formObj.s_cust_nm.value = rtnArr[2];  //eng_nm						
					}
					...
					//#3488 Accounting Multi Language 적용 
					if ( MULTI_LANGUAGE == "Y" ){
						formObj.s_paid_nm.value = rtnArr[10]; //locl_nm						
					}else{
						formObj.s_paid_nm.value = rtnArr[2];  //eng_nm						
					}
...
function popupAutomaticProcess(rtnVal){
	...
	if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
        ...
        //Use_Clear_Void옵션이 N의 경우 save할 때  deposit_dt를 Today로 셋팅
        if(useClearVoid == "N"){
     	   if (!formObj.deposit_chk.checked) {HAN_TEST_PAY
     		   formObj.deposit_chk.checked = true;
     		   formObj.deposit_chk.value = "Y";
     		   setDepositDate();
     		   depositClick();
