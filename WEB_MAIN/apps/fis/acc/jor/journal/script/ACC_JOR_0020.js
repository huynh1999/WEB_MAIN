...
function doWork(srcName){
    ...
    switch(srcName) {
	   ...
	   break;
	   case "NEW":
			var paramStr="./ACC_JOR_0010.clt?f_cmd=-1";
			...
			if("Y" == initDatLod){
				...
				if(dateValidFlag){
   					if(chkCmprPrdSc(formObj.s_post_strdt, formObj.s_post_enddt)){
   			            docObjects[0].DoSearch("./ACC_JOR_0020GS.clt", FormQueryString(formObj) );
   			            //Log Monitor Start
   				   		gLogMonitor();
   				 		//Log Monitor End
   					}
       ...
       case "MODIFY":	//수정
           ...
           if(confirm(getLabel('FMS_COM_CFMSAV'))){
        	   sheetObj.DoSave("ACC_JOR_0020GS.clt", FormQueryString(formObj),"ibflag",false);
           }
       break;
       case "DELETE":	//삭제
...
//		   }
       ...
       case "CUSTOMER_POPLIST"://openMean 1=화면에서 오픈, 2=그리드에서 오픈
	   		rtnary=new Array(1);
	   		rtnary[0]="";
	   		//#778 [Deposit List] Cannot open empty popup
...
//	       		alert("No Print Data! ");
        ...
        case "GL_CREATE_END_ACTION":
    		...
    		var slip_no		= sheetObj.GetCellValue(sRow, "jnr_no");
...
//				formObj.intg_bl_seq.value = rtnValAry[1];//intg_bl_seq
		...
		case "COPY":
		    ...
		    var paramStr="./ACC_JOR_0010.clt?f_cmd=-1&s_jnr_no="+sheetObj.GetCellValue(sRow, "jnr_no")+"&list_copy_flag=Y"+"&copySeq="+copySeq;
		    parent.mkNewFrame('Deposit', paramStr,"ACC_JOR_0010_SHEET_COPY_" + sheetObj.GetCellValue(sRow, "jnr_no"));
    ...
    switch(sheetNo) {
         case 1:      //IBSheet1 init
            with (sheetObj) {
             ...
             var cols = [ 
                 {Type:"Text",      Hidden:0,  Width:35,   Align:"Center",  ColMerge:1,   SaveName:"clt_cmpl_flg",    KeyField:0,   CalcLogic:"",   Format:"",            PointCount:1,   UpdateEdit:0,   InsertEdit:0 }, //#2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
                 ...
                 {Type:"Date",      Hidden:0,  Width:65,   Align:"Center",  ColMerge:1,   SaveName:"post_dt",       KeyField:0,   CalcLogic:"",   Format:"Ymd",         PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
                 ...
                 {Type:"Text",      Hidden:0,  Width:130,  Align:"Left",    ColMerge:1,   SaveName:"bank_seq",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
                 ...
                 {Type:"Text",      Hidden:0,  Width:100,  Align:"Center",  ColMerge:1,   SaveName:"hbl_no",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
                 ...
                 {Type:"Date",      Hidden:1, Width:65,   Align:"Center",  ColMerge:1,   SaveName:"org_post_dt",   KeyField:0,   CalcLogic:"",   Format:"Ymd",         PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
...
/*if(sheetObj.GetCellValue(i, "clt_cmpl_flg") == "Y"){
				sheetObj.SetCellValue(i, "magam_flag","Y",0);
//parameter changed[check again]CLT 				sheetObj.SetCellFontColor(i, "magam_flag","#FF0000");
	...
	//sheetObj.ShowSubSum(5, "5", 3, true, false, -1, "stockCd=%s;stockNm=%s;sTitle=소계");
		...
		*/
		...
		if(sheetObj.GetCellValue(i, "clt_cmpl_flg") == "Y" || sheetObj.GetCellValue(i, "clr_yn") == "Y" || sheetObj.GetCellValue(i, "void_yn") == "Y"){
			...
			//if(sheetObj.CellValue(i, "clt_cmpl_flg") == "Y"){
				//sheetObj.CellValue2(i, "magam_flag") = "Y";
...
if(sheetObj.GetCellValue(i, "check_flag") == "0"){
						sheetObj.SetCellValue(i, "check_flag","0",0);
					}else{
						sheetObj.SetCellValue(i, "check_flag","1",0);
					}
				} else {
					if(i != sheetObj.LastRow()){
						sheetObj.SetCellValue(i, "check_flag","0",0);
...
var paramStr="./ACC_JOR_0010.clt?f_cmd=-1&s_jnr_no="+sheetObj.GetCellValue(Row, "jnr_no");
	    parent.mkNewFrame('Customer Payment', paramStr,"ACC_JOR_0010_SHEET_" + sheetObj.GetCellValue(Row, "jnr_no"));
	}
}
...
function formValidation(){
	...
	if(!chkSearchCmprPrd(false, frm1.s_void_strdt, frm1.s_void_enddt)){
		return false;
	}
...
function excelDown(mySheet){
	...
	//#5539 [BNX] Excel All download with post date validation - POST DATE 없는 경우 검색 불가하여 제거
