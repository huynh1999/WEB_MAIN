...
function doWork(srcName){
    ...
    var sheetObj2=docObjects[2];  //#2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
  ...
  //#6734 [Zencon] POST DATE CHANGE BLOCK ROLE OPTION (Zen#3023)
    ...
    switch(srcName) {
	   ...
	   break;
	   case "NEW":
			var paramStr="./ACC_JOR_0011.clt?f_cmd=-1";
		    parent.mkNewFrame('Deposit Level 2', paramStr);	        
	   break;   
       case "SEARCHLIST":
    	   ...
    	   //sheetObj.ShowDebugMsg = true;
			...
			if("Y" == initDatLod){
				...
				if("" != formObj.s_rcv_from_cd.value 
						|| "" != formObj.s_rcv_from_nm.value 
						|| "" != formObj.s_chk_no.value) {
				...
				if(dateValidFlag){
   					if(chkCmprPrdSc(formObj.s_post_strdt, formObj.s_post_enddt)){
   			            docObjects[0].DoSearch("./ACC_JOR_0021GS.clt", FormQueryString(formObj) );
   			            //Log Monitor Start
   				   		gLogMonitor();
   				 		//Log Monitor End
   					}
...
//		   chk_cnt=arrRow.length-1;
           ...
           if(confirm(getLabel('FMS_COM_CFMSAV'))){
        	   sheetObj.DoSave("ACC_JOR_0021GS.clt", FormQueryString(formObj),"ibflag",false);
           }
       break;
       case "DELETE":	//삭제
...
//	       		alert("No Print Data! ");
        ...
        case "GL_CREATE_END_ACTION":
    		...
    		source="Deposit Journal";
    		formObj.file_name.value='account_slip_05.mrd';
			//Parameter Setting
        	var param="[" + "'" + jnr_no + "'" + ']';				// [1]
			param += '[' + source + ']';								// [2]
			param += '[' + srcNo + ']';									// [3]
		...
		//#2011 [BNX] INDIA - DEPOSIT RECEIPT
	       case "RECEIPT":
				...
				var param='[' + sheetObj.GetCellValue(sRow, "jnr_no") + ']';				// [1]
				formObj.rd_param.value=param;
				popPOST(formObj, 'RPT_RD_0020.clt', 'popTest', 1025, 740);
	      break;
...
/*    	   if(formObj.s_post_strdt.value == ""){
            ...
            //검증로직
            docObjects[1].DoSearch("./ACC_JOR_0021GS.clt", FormQueryString(formObj) );
		...
		case "COPY":
			...
			var paramStr="./ACC_JOR_0011.clt?f_cmd=-1&s_jnr_no="+sheetObj.GetCellValue(sRow, "jnr_no")+"&list_copy_flag=Y"+"&copySeq="+copySeq;
		    parent.mkNewFrame('Deposit Level 2', paramStr, "ACC_JOR_0011_SHEET_COPY_" + sheetObj.GetCellValue(sRow, "jnr_no"));			
			break;	 		
    }// end switch
...
function loadPage() {
	...
	ajaxSendPost(setOrderByInfo, 'reqVal','&goWhere=aj&bcKey=searchTbUserOrderbyInfoAttr&pgm_usr_id='+formObj.user_id.value+'&pgm_url=./ACC_JOR_0020.clt',	'./GateServlet.gsl');
    ...
    var opt_key = "SEARCH_DEPOSIT_CODE";
	ajaxSendPost(setSearchDepositCode, "reqVal", "&goWhere=aj&bcKey=searchSysOpt&opt_key="+opt_key, "./GateServlet.gsl");
...
function initSheet(sheetObj,sheetNo) {
    switch(sheetNo) {
         case 1:      //IBSheet1 init
          ...
          var headers = [ { Text:getLabel('ACC_JOR_0021_HDR'), Align:"Center"} ];
          ...
          var cols = [ {Type:"Text",      Hidden:0,  Width:35,   Align:"Center",  ColMerge:1,   SaveName:"magam_flag",    KeyField:0,   CalcLogic:"",   Format:"",            PointCount:1,   UpdateEdit:0,   InsertEdit:0 },
              ...
              {Type:"Text",      Hidden:0,  Width:250,  Align:"Left",    ColMerge:1,   SaveName:"rcvd_fm_nm",    KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
              ...
              {Type:"Text",      Hidden:0,  Width:150,  Align:"Left",    ColMerge:1,   SaveName:"bank_seq",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
              ...
              {Type:"Text",      Hidden:0,  Width:50,   Align:"Center",  ColMerge:1,   SaveName:"block",         KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
              ...
              {Type:"Text",      Hidden:1, Width:30,   Align:"Center",  ColMerge:1,   SaveName:"slip_no",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
          ...
          resizeSheet();
           ...
           break;
         case 2:      //#2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
        	 with(sheetObj){
         ...
         <!-- ############################################### COMMON MEMO 2-4 ##################################################### -->
           case 3:      //IBSheet1 init   #2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
        	 initMemo(sheetObj);                                              
	       break;
	     <!-- ############################################### COMMON MEMO 2-4 ##################################################### -->
     }
...
function sheet2_OnSearchEnd(sheetObj, errMsg){
	...
	for(var i=1; i<=sheetObj.LastRow();i++){
		//마감이나 PAID됐을시 POST_DT를 수정불가하게 한다.
		if(sheetObj.GetCellValue(i, "clt_cmpl_flg") == "Y" || sheetObj.GetCellValue(i, "clr_yn") == "Y" || sheetObj.GetCellValue(i, "void_yn") == "Y"){
...
if(sheetObj.GetCellValue(i, "clt_cmpl_flg") == "Y"){
	...
	setGridOfcBlckFlg(sheetObj);  //#2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
...
//				
//				if(i == Row){
//					if(sheetObj.CellValue(i, "check_flag") == "0"){
//						sheetObj.CellValue(i, "check_flag") = "0";
//					}else{
//						sheetObj.CellValue(i, "check_flag") = "1";
//					}
//				}else{
//					if(i != sheetObj.LastRow){
//						sheetObj.CellValue(i, "check_flag") = "0";
...
function sheet1_OnDblClick(sheetObj,Row,Col){
	...
	if(sheetObj.ColSaveName(Col) != "post_dt" && sheetObj.ColSaveName(Col) != "check_flag"){
		var paramStr="./ACC_JOR_0011.clt?f_cmd=-1&s_jnr_no="+sheetObj.GetCellValue(Row, "jnr_no");
	    parent.mkNewFrame('Deposit Level 2', paramStr, "ACC_JOR_0011_SHEET_" + sheetObj.GetCellValue(Row, "jnr_no"));
	}
}
...
function clearAll(){
	...
	setOfficeAllOption(formObj.s_ofc_cd);
	setSelection();
	//formObj.s_bank_cd[1].selected=true;
	...
	if(doc[0]=='OK'){
		if(typeof(doc[1])!='undefined'){
			...
			if(CODETYPE =="BILLTO"){
				...
				if(MULTI_LANGUAGE == 'Y'){
					formObj.s_rcv_from_nm.value=masterVals[16];		//eng_nm   AS param2
				}else {
...
function CUSTOMER_POPLIST(rtnVal){
	...
	if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
		...
		//#3488 Accounting Multi Language 적용
		if(MULTI_LANGUAGE == 'Y'){
			formObj.s_rcv_from_nm.value=rtnValAry[10];//full_nm
		}else {
			formObj.s_rcv_from_nm.value=rtnValAry[2];//full_nm
		}
	}             
...
function getStatus(row){
	var sheetObj=docObjects[0];
	if (sheetObj.GetCellValue(row, "rcvd_fm_nm") == changeValue){
		return false;
	}else{
		return true;
	}
...
function setGridOfcBlckFlg(sheetObj){  //#2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
 
	var formObj=document.frm1;
	//var sheetObj=docObjects[0];      //#2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
	/*
 	...
 	if (apo_flg == "N"){
		for(var i=1; i<=sheetObj.LastRow();i++){ 
			...
			if (change == true){
				...
				sheetObj.SetCellValue(i, "rcvd_fm_nm",changeValue);
				//sheetObj.SetCellValue(i, "chk_no",changeValue);
				sheetObj.SetCellValue(i, "bank_seq",changeValue);
				...
				sheetObj.SetCellValue(i, "p_ofc_cd",changeValue);
...
var searchDepositCode = '';
function setSearchDepositCode(reqVal){
	var doc=getAjaxMsgXML(reqVal);

	if (doc[0]=="OK" && typeof doc[1] != "undefined" ){
		searchDepositCode = doc[1];
	}
	if(searchDepositCode != 'Y'){
		frm1.s_search_rcv_type.disabled = true;
	}
}