...
//  	       		alert("No Print Data! ");
		...
		case "EXCEL_ALL":
   	   ...
   	   //sheetObj.ShowDebugMsg = true;
           ...
           //검증로직
           docObjects[1].DoSearch("./ACC_JOR_0042GS.clt", FormQueryString(formObj) );
		...
		case "COPY":
			...
			var paramStr="./ACC_JOR_0032.clt?f_cmd=-1&s_jnr_no="+sheetObj.GetCellValue(sRow, "jnr_no")+"&list_copy_flag=Y"+"&copySeq="+copySeq;
		    parent.mkNewFrame('Payment Level 3', paramStr,"ACC_JOR_0032_SHEET_COPY_" + sheetObj.GetCellValue(sRow, "jnr_no"));			
			break;	      	
		...
		case "BATCH_PRINT":
	        ...
	        callBackFunc = "";
	        modal_center_open('./ACC_JOR_0043.clt', rtnary, 400,380,"yes");  //3345 #1106 개선사항 
	      break;					
    }// end switch
...
//			getBtnObj('riderprintBtn02').style.display="none";
		...
		if(sheetObj.GetCellValue(Row, "rider_yn") == "Y"){
			...
			// #3655 [SUNWAY] CANNOT SELECT ITEM ON PAYMENT LIST LEVEL 3
			//getBtnObj('riderprintBtn02').style.display="none";
			formObj.f_jnr_no.value=sheetObj.GetCellValue(Row, "jnr_no");
			formObj.rider_yn.value="N";
		}
...
function clearAll(){
	...
	/* Comment these lines to set blank for Post Date textbox when user click "Clear" button.
	formObj.s_post_strdt.value=FROMDATE;
	formObj.s_post_enddt.value=TODAY;*/
	...
	setSelection();
	//formObj.s_bank_cd[2].selected=true; // formObj.s_bank_cd[2].value = "PAYMENT BANK";
