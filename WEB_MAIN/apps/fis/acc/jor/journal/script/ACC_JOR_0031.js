//=========================================================
//*@FileName   : ACC_JOR_0031.jsp
//*@FileTitle  : Check Journal Manager
//*@Description: Check Journal Manager
   	   ...
   	   case "PRINT_CALLBACK":
  		   	...
  		   	// LHK, 20140307, #26818 [BINEX]Check Print 이후 Payment 자료 수정이 되고 있음 
   	   		if (isPrinted) {
	   	   		...
	   	   		docObjects[0].DoSearch("./ACC_JOR_0031GS.clt", FormQueryString(formObj) );
			}
  	   break;
   	   case "CLEAR":
       	   ...
       	   if(chekVal != 'PRNSAVE'  && !checkDelete()){
	           ...
	           if(saveCfmFlg){
	    		   ...
	    		   if(saveFlg){
			           ...
			           sheetObj.DoAllSave("./ACC_JOR_0031GS.clt", FormQueryString(formObj)+'&'+sht2, true);
	    		   }
	           }
    	   }
    	   break;
...
//	   		rtnary[2]=window;
      ...
      case "CUSTOMER2_NAME":
       ...
       case "PRINT":
       		if(formObj.f_lst_chk_no.value != "" && formObj.f_cur_chk_no.value == formObj.f_chk_no.value &&
    	  		eval(formObj.f_cur_chk_no.value) > eval(formObj.f_lst_chk_no.value) ){	//current check no 가 Last check no 보다 클 경우 경고
    	  		if(!confirm(getLabel('FMS_COM_ALT004') ) ){
    	  			return;
    	  		}
    	    }
			...
			//저장이 Success 인경우 (formObj.print_auto_save.value 는 'S') 만 Print 함
			/* 동기화 문제 때문에 위치 변경
...
//	   			DownCols: makeHiddenSkipCol(sheetObj)
	   			sheetObj.Down2Excel( {SheetDesign:1,Merge:1, CheckBoxOnValue:'Y', CheckBoxOffValue:'', HiddenColumn:1});
	   		}
       break;
    }
}
    ...
    switch(sheetNo) {
         case 1:      //IBSheet1 init
            with (sheetObj) {
	          ...
	          var cols = [
	              {Type:"CheckBox",  Hidden:0,  Width:30,   Align:"Center",  ColMerge:1,   SaveName:"del_chk",           KeyField:0,   CalcLogic:"",   Format:"",            PointCount:1,   UpdateEdit:1,   InsertEdit:1, HeaderCheck:0 },
	              ...
	              {Type:"Text",      Hidden:0,  Width:110,  Align:"Center",  ColMerge:1,   SaveName:"buy_inv_no",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
...
function sheet1_OnSearchEnd(){
	...
	if(sheetObj.GetCellValue(1,"jnr_no") != "" ){
		...
		formObj.office_block_chk.disabled  = false;
		if(sheetObj.GetCellValue(1, "ofc_blck_flg") == "2" || sheetObj.GetCellValue(1, "ofc_blck_flg") == "3"){ // blck 3 인것도 체크 추가 16.03.24
			formObj.office_block_chk.checked=true;		
			if (formObj.f_ofc_cd.value != formObj.f_usr_ofc_cd.value){ // 다르면 체크 해제 불가 
				formObj.office_block_chk.disabled  = true;
			}		
		}else{
			formObj.office_block_chk.checked=false;
		}			
		//alert(formObj.f_ofc_cd.value + " 11111111 " +formObj.f_usr_ofc_cd.value);
