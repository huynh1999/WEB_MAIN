...
function doWork(srcName){
    ...
    switch(srcName) {
	   ...
	   break;
	   case "NEW":
			var paramStr="./ACC_JOR_0031.clt?f_cmd=-1";
		    parent.mkNewFrame('Payment Level 2', paramStr);
	   break;     
       case "SEARCHLIST":
			...
			if("Y" == initDatLod){
				...
				if(dateValidFlag){
   					if(chkCmprPrdSc(formObj.s_post_strdt, formObj.s_post_enddt)){
   			            docObjects[0].DoSearch("./ACC_JOR_0041GS.clt", FormQueryString(formObj) );
   			            //Log Monitor Start
   				   		gLogMonitor();
   				 		//Log Monitor End
   					}
...
//		   chk_cnt=arrRow.length-1;
           ...
           if(confirm(getLabel('FMS_COM_CFMSAV'))){
        	   sheetObj.DoSave("ACC_JOR_0041GS.clt", FormQueryString(formObj),"ibflag",false);
           }
           break;
       case "DELETE":	//삭제
...
// 	       		alert("No Print Data! ");
	...
	case "EXCEL_ALL":
	  ...
	  //#5539 [BNX] Excel All download with post date validation - Skip the post date validation
	   ...
	   //검증로직
	   docObjects[1].DoSearch("./ACC_JOR_0041GS.clt", FormQueryString(formObj) );
	...
	case "COPY":
		...
		var paramStr="./ACC_JOR_0031.clt?f_cmd=-1&s_jnr_no="+sheetObj.GetCellValue(sRow, "jnr_no")+"&list_copy_flag=Y"+"&copySeq="+copySeq;
	    parent.mkNewFrame('Payment Level 2', paramStr, "ACC_JOR_0031_SHEET_COPY_" + sheetObj.GetCellValue(sRow, "jnr_no"));	    
		break;	 
...
function initSheet(sheetObj,sheetNo) {
    switch(sheetNo) {
         ...
         case 2:      //#2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
           ...
           <!-- ############################################### COMMON MEMO 2-4 ##################################################### -->
	       case 3:      //Memo IBSheet init  #2374 [LBS] Deposit/Payment List 에 Excel (All) 버튼 추가
	      	 initMemo(sheetObj);                                              
	       break;
	       <!-- ############################################### COMMON MEMO 2-4 ##################################################### -->
     }
...
function sheet1_OnDblClick(sheetObj,Row,Col){
	...
	if(sheetObj.ColSaveName(Col) != "post_dt" && sheetObj.ColSaveName(Col) != "check_flag"){
		var paramStr="./ACC_JOR_0031.clt?f_cmd=-1&s_jnr_no="+sheetObj.GetCellValue(Row, "jnr_no");
	    parent.mkNewFrame('Payment Level 2', paramStr, "ACC_JOR_0031_SHEET_" + sheetObj.GetCellValue(Row, "jnr_no"));
	}
}
...
function clearAll(){
	...
	/* [#1275][Payment Lv2 List] Clear function does not reset search conditions to default value.
	formObj.s_post_strdt.value=FROMDATE;
	formObj.s_post_enddt.value=TODAY;
	*/
	...
	setSelection();
	//formObj.s_bank_cd[2].selected=true; // [#1275][Payment Lv2 List] Clear function does not reset search conditions to default value.
	...
	// alert(ofc_cd + " "+apo_flg+" "+sheetObj.LastRow()+" "+deposit_level+" "+payment_level);
 	if (apo_flg == "N"){
		for(var i=1; i<=sheetObj.LastRow();i++){
			...
			var level = sheetObj.GetCellValue(i, "level_value");
			...
			var change = false;
			//alert("level_value "+ level + " ofc_blck_flg"+ofc_blck_flg + " ofc_cd"+ofc_cd +" p_ofc_cd"+p_ofc_cd );
			//기존 블럭에 라벨권한 추가 2016.03.25 
			if (payment_level == "2" && payment_level == level  && ofc_blck_flg >= payment_level && p_ofc_cd != ofc_cd){ 
				change = true;
			}   else if (payment_level == "2" && payment_level < level){ 
				change = true;
			}   else if (payment_level == "3" && level == "2"  && ofc_blck_flg >= payment_level && p_ofc_cd != ofc_cd){  
				change = true;
			}   else if (payment_level == "3" && level == "3"  && ofc_blck_flg == payment_level && p_ofc_cd != ofc_cd){ 
				change = true;
			}   
