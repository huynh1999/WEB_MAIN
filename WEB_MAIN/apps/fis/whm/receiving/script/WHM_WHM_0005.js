...
//			param += '[123]';
			param += '['+getPoNo()+']';
			param += '['+whCd+']';
			param += '[19515 S.VERMONTAVE. TORRANCE,CA 90502]';
			if(formObj.f_len_ut_cd[0].checked){
				param += '[L X W X H (Cm)]';
			}else{
				param += '[L X W X H (Inch)]';
			}
...
function OpenCustItmPop(colValue){
	...
	var sUrl="CtrtItemPopup.clt?ctrt_no="+formObj.s_ctrt_no.value+"&item_cd="+encodeURIComponent(colValue);
    ...
    switch(sheetNo) {
         case 1:      //IBSheet1 init
            with (sheetObj) {
	           ...
	           var HeadTitle1 = "|DEL|Item|Item|Unit|Inner Qty.|Ctn.|EA|Total Qty|Location|Item Description|P/O No.|L|W|H|Dim Weight|Dim Weight|Actual Weight|Actual Weight|Volume|Volume" ;
	           var HeadTitle2 = "|DEL|Item|Item|Unit|Inner Qty.|Ctn.|EA|Total Qty|Location|Item Description|P/O No.|L|W|H|KG|LB|KG|LB|CBM|CFT" ;
...
//	           var HeadTitle2 = getLabel('WHM_WHM_0005_HDR2') ;
	           ...
	           var cols = [   {Type:"Status",    Hidden:1,  Width:20,   Align:"Center",  ColMerge:1,   SaveName:"ibflag" },
			                  ...
			                  {Type:"AutoSum",   Hidden:0,  Width:40,   Align:"Right",   ColMerge:1,   SaveName:"itm_ea_qty",    KeyField:0,   CalcLogic:"",   Format:"Integer",     PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:7 },
			                  ...
			                  {Type:"AutoSum",   Hidden:0,  Width:85,   Align:"Right",   ColMerge:1,   SaveName:"itm_dim_wgt",   KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:3,   UpdateEdit:1,   InsertEdit:1,   EditLen:12 },
			                  {Type:"AutoSum",   Hidden:0,  Width:85,   Align:"Right",   ColMerge:1,   SaveName:"itm_dim_wgt_lbs",  KeyField:0,   CalcLogic:"",   Format:"Float",    PointCount:3,   UpdateEdit:1,   InsertEdit:1,   EditLen:18 },
			                  {Type:"AutoSum",   Hidden:0,  Width:85,   Align:"Right",   ColMerge:1,   SaveName:"itm_act_wgt",      KeyField:0,   CalcLogic:"",   Format:"Float",    PointCount:3,   UpdateEdit:1,   InsertEdit:1,   EditLen:12 },
			                  {Type:"AutoSum",   Hidden:0,  Width:85,   Align:"Right",   ColMerge:1,   SaveName:"itm_act_wgt_lbs",  KeyField:0,   CalcLogic:"",   Format:"Float",    PointCount:3,   UpdateEdit:1,   InsertEdit:1,   EditLen:18 },
			                  {Type:"AutoSum",   Hidden:0,  Width:85,   Align:"Right",   ColMerge:1,   SaveName:"itm_vol",          KeyField:0,   CalcLogic:"",   Format:"Float",    PointCount:3,   UpdateEdit:1,   InsertEdit:1,   EditLen:12 },
			                  {Type:"AutoSum",   Hidden:0,  Width:85,   Align:"Right",   ColMerge:1,   SaveName:"itm_vol_cft",      KeyField:0,   CalcLogic:"",   Format:"Float",    PointCount:3,   UpdateEdit:1,   InsertEdit:1,   EditLen:18 },
...
function sheet1_OnPopupClick(sheetObj, Row, Col) {
	...
	if(colName == "cust_itm_id"){
		...
		var sUrl="CommonCodePopup.clt?grp_cd=A6&code="+colValue+"&wh_flag=Y&ctrt_no="+ComGetObjValue(formObj.s_ctrt_no) +"&item_sys_no="+sheetObj.GetCellValue(Row, "item_sys_no");
...
//	fnCalcItemEaQty(sheetObj, sheetObj.GetSelectRow(), sheetObj.GetSelectCol());
	ajaxSendPost(getInfo_Item_byCtrtNo_UnitCd, 'reqVal', '&goWhere=aj&bcKey=getInfo_Item_byCtrtNo_UnitCd&ctrt_no='+formObj.s_ctrt_no.value+"&unit_cd="+arrVal[1]+"&itm_cd="+ sheet1.GetCellValue(sheet1.GetSelectRow(), "cust_itm_id"), './GateServlet.gsl');
}
	...
	if(doc[0]=='OK'){
		if(typeof(doc[1])!='undefined'){
			...
			if(rtnArr[0] != "null" && rtnArr[0] != ""){
				...
				if(arrVal.length > 2){
					var row = sheet1.GetSelectRow();
					sheet1.SetCellValue(row,"itm_len",arrVal[1],0);
					sheet1.SetCellValue(row,"itm_wdt",arrVal[2],0);
...
function displayData(xml){
	  ...
	  $("#s_ctrt_no").val($xml.find( "ctrt_no").text());
...
function ValidateForm(){
	...
	for(i = 2; i< sheet1.LastRow(); i++){
		for(var j = i+1; j< sheet1.LastRow();j++){
			if(sheet1.GetCellValue(i,"cust_itm_id") == sheet1.GetCellValue(j,"cust_itm_id")){
...
//				break;
			...
			case "itm_hgt":
				...
				if(formObj.f_len_ut_cd[0].checked){
					...
					sheetObj.SetCellValue(row, "itm_dim_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(2),0);
					sheetObj.SetCellValue(row, "itm_act_wgt",kg.toFixed(3),0);
					sheetObj.SetCellValue(row, "itm_act_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(2),0);
					sheetObj.SetCellValue(row, "itm_vol",cbm.toFixed(6),0);
				...
				break;
			case "itm_dim_wgt":
				sheetObj.SetCellValue(row, "itm_dim_wgt_lbs",roundXL(sheetObj.GetCellValue(row, col) * CNVT_CNST_KG_LB, 2),0);
				break;
			case "itm_dim_wgt_lbs":
				sheetObj.SetCellValue(row, "itm_dim_wgt",roundXL(sheetObj.GetCellValue(row, col) / CNVT_CNST_KG_LB, 2),0);
				break;
			case "itm_act_wgt":
				sheetObj.SetCellValue(row, "itm_act_wgt_lbs",roundXL(sheetObj.GetCellValue(row, col) * CNVT_CNST_KG_LB, 2),0);
				break;
			case "itm_act_wgt_lbs":
				sheetObj.SetCellValue(row, "itm_act_wgt",roundXL(sheetObj.GetCellValue(row, col) / CNVT_CNST_KG_LB, 2),0);
				break;
			case "itm_vol":
				sheetObj.SetCellValue(row, "itm_vol_cft",roundXL(sheetObj.GetCellValue(row, col) *35.3147, 2),0);
				break;
			case "itm_vol_cft":
				sheetObj.SetCellValue(row, "itm_vol",roundXL(sheetObj.GetCellValue(row, col) /35.3147, 2),0);
				break;
			case "cb_loc_id":
...
function chkSizeType(){
	...
	for(var i=2 ; i<sheetObj.LastRow(); i++){
		...
		if(formObj.f_len_ut_cd[0].checked){
			...
			kg=roundXL(length * width * height * pcs / 6000, 3);
			cbm=roundXL(length * width * height * pcs * 0.01 * 0.01 * 0.01, 6);
			sheetObj.SetCellValue(i, "itm_dim_wgt",kg.toFixed(3),0);
			sheetObj.SetCellValue(i, "itm_dim_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(2),0);
			sheetObj.SetCellValue(i, "itm_act_wgt",kg.toFixed(3),0);
			sheetObj.SetCellValue(i, "itm_act_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(2),0);
			sheetObj.SetCellValue(i, "itm_vol",cbm.toFixed(6),0);
			sheetObj.SetCellValue(i, "itm_vol_cft",(cbm * 35.3147).toFixed(3),0);
		}else if(formObj.f_len_ut_cd[1].checked){
			// 인치 
			kg=roundXL(length * width * height * pcs * 2.54 * 2.54 * 2.54 / 6000, 3);
			cbm=roundXL(length * width * height * pcs * 0.0254 * 0.0254 * 0.0254, 6);
...
function loadSizeType(){
	...
	for(var i=2 ; i<sheetObj.LastRow(); i++){
		...
		if(formObj.f_len_ut_cd[0].checked){
			...
			kg=roundXL(length * width * height * pcs / 6000, 3);
			cbm=roundXL(length * width * height * pcs * 0.01 * 0.01 * 0.01, 6);
			sheetObj.SetCellValue(i, "itm_dim_wgt",parseFloat(sheetObj.GetCellValue(i, "itm_dim_wgt"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_dim_wgt")) : kg.toFixed(3),0);
			sheetObj.SetCellValue(i, "itm_dim_wgt_lbs",parseFloat(sheetObj.GetCellValue(i, "itm_dim_wgt_lbs"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_dim_wgt_lbs")) :(kg * CNVT_CNST_KG_LB).toFixed(2) ,0);
			sheetObj.SetCellValue(i, "itm_act_wgt",parseFloat(sheetObj.GetCellValue(i, "itm_act_wgt"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_act_wgt")) : kg.toFixed(3),0);
			sheetObj.SetCellValue(i, "itm_act_wgt_lbs",parseFloat(sheetObj.GetCellValue(i, "itm_act_wgt_lbs"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_act_wgt_lbs")) :(kg * CNVT_CNST_KG_LB).toFixed(2) ,0);
			sheetObj.SetCellValue(i, "itm_vol",parseFloat(sheetObj.GetCellValue(i, "itm_vol"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_vol")) :cbm.toFixed(6), 0);
			sheetObj.SetCellValue(i, "itm_vol_cft",parseFloat(sheetObj.GetCellValue(i, "itm_vol_cft"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_vol_cft")) :(cbm * 35.3147).toFixed(3),0);
		}else if(formObj.f_len_ut_cd[1].checked){
			// 인치 
			kg=roundXL(length * width * height * pcs * 2.54 * 2.54 * 2.54 / 6000, 3);
			cbm=roundXL(length * width * height * pcs * 0.0254 * 0.0254 * 0.0254, 6);
