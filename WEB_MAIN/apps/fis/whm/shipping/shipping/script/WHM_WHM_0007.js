...
//    	   rtnary[1]=formObj.splr_rcvr_nm.value;
    	   ...
    	   modal_center_open('./CMM_POP_0010.clt', rtnary, 1150,650,"yes");
    	   break;
       case "SU_POPLIST_NAME"://openMean 1=화면에서 오픈, 2=그리드에서 오픈
    	   rtnary=new Array(1);
    	   rtnary[0]="1";
    	   rtnary[1]=formObj.splr_rcvr_nm.value;
    	   rtnary[2]=window;
...
//    	   rtnary[1]=formObj.trkr_nm.value;
    	   ...
    	   modal_center_open('./CMM_POP_0010.clt', rtnary, 1150,650,"yes");
    	   break;
       case "TR_POPLIST_NAME"://openMean 1=화면에서 오픈, 2=그리드에서 오픈
    	   rtnary=new Array(1);
    	   rtnary[0]="1";
    	   rtnary[1]=formObj.trkr_nm.value;
    	   rtnary[2]=window;
       ...
       case "INVT_ADD"://사용자조회 openMean 1=화면에서 오픈, 2=그리드에서 오픈
    	   if(formObj.cust_cd.value!=""){
    		   if(formObj.wh_cd.value!=""){
    			   loadWh();
    		   } else {
//    			   alert("Please select Warehouse before add new items !");
    			   ComShowCodeMessage('COM12245');
       ...
       case "NEW":
		...
		case "ROWADD":
			...
			if(formObj.cust_cd.value!=""){
				...
				formObj.s_ctrt_no.disabled =1;
				formObj.btn_ctrt_no.disabled = 1;
				sheet1_OnChange(sheet1,intRows,3,"");
			}else{
//				ComShowCodeMMessage('COM12246');
////				alert("Please select Customer before add new items !");
//				formObj.cust_cd.focus();
				if(formObj.s_ctrt_no.value==""){
...
//				alert("Please select Customer before add new items !");
		...
		case "PRINT":
			...
			f_wh_recp_no=formObj.cre_file_no.value;
			i_cust_cd = formObj.cust_cd.value;
			i_ship_cd = formObj.splr_rcvr_cd.value;
			trucker = formObj.trkr_cd.value;
			repno = formObj.cust_ref_no.value;
			regis = formObj.cre_ofc_cd_list.value;
...
function RECP_WHINVT_POPLIST(rtnVal){
	var formObj=document.frm1;
	if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
		return;
	}else{
...
/*function OpenCustItmPop(colValue){
	var formObj=document.frm1;
	 rtnary=new Array(1);
	   ...
	   rtnary[0]="1";
	   rtnary[1]=formObj.cust_cd.value;
	   rtnary[2]=formObj.cust_nm.value;
	   rtnary[3]=window;
...
function OpenCustItmPop(colValue){
    ...
    callBackFunc = "OPEN_CUST_ITM_POP";
	var sUrl="CtrtItemPopup.clt?ctrt_no="+encodeURIComponent(formObj.s_ctrt_no.value)+"&item_cd="+encodeURIComponent(colValue);
	...
	var formObj = document.frm1;
	   if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
		 ...
		 //  var itmId = sheet1.GetCellValue(row,"cb_itm_nm");
		   ...
		   sheet1.SetCellValue(row,"item_sys_no",rtnValAry[3],0);
    ...
    switch(sheetNo) {
         case 1:      //IBSheet1 init
            with (sheetObj) {
	           ...
	           var HeadTitle2 = "|DEL|Item|Item|Unit|Inner Qty.|Qty.|EA|Total Qty|Location|Item Description|P/O No.|L|W|H|KG|LB|KG|LB|CBM|CFT" ;
	           ...
	           var cols = [   {Type:"Status",    Hidden:1,  Width:20,   Align:"Center",  ColMerge:1,   SaveName:"ibflag" },
			                  ...
			                  {Type:"Popup",     Hidden:0,  Width:80,   Align:"Center",  ColMerge:1,   SaveName:"cust_itm_id",   	KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 },
			                  ...
			                  {Type:"Popup",     Hidden:0,  Width:70,   Align:"Center",  ColMerge:1,   SaveName:"cb_ut_cd",      	KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:7 },
			                  {Type:"Int",       Hidden:0,  Width:70,   Align:"Right",   ColMerge:1,   SaveName:"itm_inr_qty",   	KeyField:1,   CalcLogic:"",   Format:"Integer",     PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:7 },
			                  ...
			                  {Type:"Text",      Hidden:0,  Width:70,   Align:"Left",    ColMerge:1,   SaveName:"itm_po_no",     	KeyField:0,   CalcLogic:"",   Format:"",       	 PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:10 },
...
//					sheet1.SetCellValue(row, "itm_dim_wgt_lbs",roundXL(arrVal[4] * CNVT_CNST_KG_LB, 2),0);
//					sheet1.SetCellValue(row,'itm_act_wgt',arrVal[5],0);
//					sheet1.SetCellValue(row, "itm_act_wgt_lbs",roundXL(arrVal[5] * CNVT_CNST_KG_LB, 2),0);
...
function displayData(xml){
	  ...
	  for(var i=0; i< formObj.f_len_ut_cd.length; i++){
	...
	//set select dropdown warehouse
	  ...
	  if(flg == "Y"){
		  formObj.btn_invt_add.disabled = true;
		  formObj.btn_add.disabled = true;
	  }else{
		  formObj.btn_invt_add.disabled = false;
		  formObj.btn_add.disabled = false;
	  }
...
function clearData(){
	...
	formObj.s_ctrt_no.disabled =0;
	formObj.btn_ctrt_no.disabled = 0;
...
function doSave(question){
	...
	if (r == true) {
		...
		if(checkFileNo){
			...
			formObj.file_no.value = formObj.cre_file_no.value;
			formObj.ofc_cd_list.value = formObj.cre_ofc_cd_list.value; 
			doSearchReceiving();
			statusflag = "U";
		}else{
...
function ValidateForm(){
	...
	//#3322-[UCB] [WMS3.0] SHIPPING ITEM DELETED, BUT STILL SHOWS AS SHIPPED
	var chkIbflag ="";
	var chkRcv_shp_flg = formObj.rcv_shp_flg.checked?"Y":"N";
	for(var i = 2; i< sheet1.LastRow(); i++){
		//#3322-[UCB] [WMS3.0] SHIPPING ITEM DELETED, BUT STILL SHOWS AS SHIPPED
		if(sheet1.GetCellValue(i,"ibflag")=="D"){
			if(chkRcv_shp_flg =="Y"){
				ComShowCodeMessage('COM12249');
				//alert('Shipped processing and item deletion can not be processed simultaneously.\n\nDelete the Item first and process it Shipped.');
				return;
			}
		}
...
function GetSaveData(){
	...
	//PQC BUG #1370 [WH Shipping Entry] System is processing after saving data
	if(formObj.estm_rcv_dt.value !=""){
...
function doSearchReceiving(){
	...
	if(formObj.file_no.value.split() == "" ){
		//PQC BUG #1370 [WH Shipping Entry] Show wrong message when searching without data at search field
		ComShowCodeMessage('COM12221');
//		alert("Please enter search condition(s).");
		clearData();
		...
		if(checkFileNo == false){
			ComShowCodeMessage('COM12252');
//			alert("Shipping No. entered does not exist. ");
			clearData();
			formObj.file_no.focus();
			return;
		}
	...
	doShowProcess();
	var params = "?f_cmd="+SEARCH + "&file_no="+formObj.file_no.value + "&rgst_ofc_cd="+formObj.ofc_cd_list.value+'&rcv_shp_tp_cd=SHP';
...
function doSearchReceivingItem(){
	var formObj = document.frm1;
	var params = "f_cmd="+SEARCH04 + "&file_no="+formObj.cre_file_no.value + "&rgst_ofc_cd="+formObj.cre_ofc_cd_list.value+'&rcv_shp_tp_cd=SHP';
//	var xml = 
...
function sheet1_OnChange(sheetObj,row,col,val){
	...
	var formObj=document.frm1;
		switch(colNm){
			//PQC BUG #1370 [WH Shipping Entry] Data at Name field still not be delete when delete code
			case "cust_itm_id":
				//sheet1.SetCellValue(row,"cb_itm_nm",sheet1.GetCellValue(row,"cust_itm_id"));
				sheet1.SetCellValue(row,"cb_itm_nm", "");
			break;
//			case "cb_itm_nm":
//				var itmId = sheet1.GetCellValue(row,"cb_itm_nm");
...
//				break;
			...
			case "itm_hgt":
				setTotal(row);
				//#1118 - [WH Shipping Entry] IBSheet Text Field Maxlength validation- Can't close message after click OK button
				// Additional validation Unit (length / Height / Width) (0- 999.99) sync with Item's unit on Item Management screen.
				if(colNm == "itm_len" || colNm == "itm_wdt"|| colNm == "itm_hgt"){
					var chkitem = sheetObj.GetCellValue(row, col).split(".");
					if(chkitem[0] == "") return;
					if(parseInt(chkitem[0]) > 999){
						sheetObj.SetCellValue(row, col, 0, 0);
						ComShowCodeMessage('COM0786', "Length / Width / Height");
					}
				}
...
//				var pcs=sheetObj.GetCellValue(row, "itm_total_qty")=="" ? 0 : sheetObj.GetCellValue(row, "itm_total_qty");
...
//				if(formObj.f_len_ut_cd[0].checked){
					...
					sheetObj.SetCellValue(row, "itm_dim_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(5),0);
					sheetObj.SetCellValue(row, "itm_act_wgt",kg.toFixed(5),0);
					sheetObj.SetCellValue(row, "itm_act_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(5),0);
					sheetObj.SetCellValue(row, "itm_vol",cbm.toFixed(5),0);
					sheetObj.SetCellValue(row, "itm_vol_cft",(cbm * 35.3147).toFixed(3),0);
...
//				}else if(formObj.f_len_ut_cd[1].checked){
					...
					sheetObj.SetCellValue(row, "itm_dim_wgt",kg.toFixed(3),0);
					sheetObj.SetCellValue(row, "itm_dim_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(5),0);
					sheetObj.SetCellValue(row, "itm_act_wgt",kg.toFixed(5),0);
					sheetObj.SetCellValue(row, "itm_act_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(5),0);
					sheetObj.SetCellValue(row, "itm_vol",cbm.toFixed(5),0);
				...
				break;
			case "itm_dim_wgt":
				sheetObj.SetCellValue(row, "itm_dim_wgt_lbs",roundXL(sheetObj.GetCellValue(row, col) * CNVT_CNST_KG_LB, 5),0);
				break;
			case "itm_dim_wgt_lbs":
				sheetObj.SetCellValue(row, "itm_dim_wgt",roundXL(sheetObj.GetCellValue(row, col) / CNVT_CNST_KG_LB, 5),0);
				break;
			case "itm_act_wgt":
				sheetObj.SetCellValue(row, "itm_act_wgt_lbs",roundXL(sheetObj.GetCellValue(row, col) * CNVT_CNST_KG_LB, 5),0);
				break;
			case "itm_act_wgt_lbs":
				sheetObj.SetCellValue(row, "itm_act_wgt",roundXL(sheetObj.GetCellValue(row, col) / CNVT_CNST_KG_LB, 5),0);
				break;
			case "itm_vol":
...
function checkDupInsertFileNo(){
	...
	if(formObj.cre_file_no.value != "" && formObj.cre_file_no.value != "AUTO"){
		checkDuplicateFileNo("SAVE");
		if(checkFileNo){
			ComShowCodeMessage('COM12241','Shipping');
//			alert("Duplicated Data!\n" +
//			"W/H Shipping file No.");
			formObj.cre_file_no.value = "AUTO";
		}
	}
}
...
function checkDuplicateFileNo(doWork){
	var formObj=document.frm1;
	if(doWork == "SEARCH"){
		ajaxSendPost(checkDuplicateFileNoEnd, 'reqVal', '&goWhere=aj&bcKey=checkDuplicateFileNo&file_no='+formObj.file_no.value+'&rcv_shp_tp_cd=SHP', './GateServlet.gsl');
	}
	if(doWork == "SAVE"){
		ajaxSendPost(checkDuplicateFileNoEnd, 'reqVal', '&goWhere=aj&bcKey=checkDuplicateFileNo&file_no='+formObj.cre_file_no.value+'&rcv_shp_tp_cd=SHP', './GateServlet.gsl');
	}
}
...
function checkRollback(){
	...
	if(!formObj.rcv_shp_flg.checked){
		...
		if (check_new != ""){
			...
			param += "cust_cd=" + formObj.cust_cd.value;
			param += "&wh_cd=" + formObj.wh_cd.value;
			param += "&rcv_shp_dt=" + tmp;
			var sXml = sheet1.GetSearchData("./WHM_WHM_0005_08GS.clt", param + "&f_cmd=" + MODIFY);
			var xmlDoc = $.parseXML(sXml);
			var $xml = $(xmlDoc);
			var in_cnt = $xml.find( "in_cnt").text();
			if(in_cnt != ""){
				ComShowCodeMessage('COM1510086');
		...
		if(originRcvFlg == 1){			
			var r = confirm("Do you want to cancel Shipping request ?");
			if (!r) {
...
//		if(formObj.f_len_ut_cd[0].checked){
			...
			sheetObj.SetCellValue(i, "itm_dim_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(5),0);
			sheetObj.SetCellValue(i, "itm_act_wgt",kg.toFixed(5),0);
			sheetObj.SetCellValue(i, "itm_act_wgt_lbs",(kg * CNVT_CNST_KG_LB).toFixed(5),0);
...
//			if(formObj.f_len_ut_cd[0].checked){
			...
			sheetObj.SetCellValue(i, "itm_dim_wgt_lbs",parseFloat(sheetObj.GetCellValue(i, "itm_dim_wgt_lbs"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_dim_wgt_lbs")) :(kg * CNVT_CNST_KG_LB).toFixed(5) ,0);
			...
			sheetObj.SetCellValue(i, "itm_act_wgt_lbs",parseFloat(sheetObj.GetCellValue(i, "itm_act_wgt_lbs"))!=0 ? parseFloat(sheetObj.GetCellValue(i, "itm_act_wgt_lbs")) :(kg * CNVT_CNST_KG_LB).toFixed(5) ,0);
...
function setControlStatus(action){
	...
	switch(action){
		...
		case "CLEAR":
			...
			formObj.btn_invt_add.disabled = 0;
...
function loadWh(){	
	rtnary=new Array(9);
	rtnary[0]=frm1.wh_cd.value;
	rtnary[1]=frm1.cust_cd.value
	//rtnary[2]=frm1.ofc_cd_cust.value;
	rtnary[2]= frm1.cre_ofc_cd_list.value;
	rtnary[3]=frm1.s_ctrt_no.value;
	callBackFunc = "rtnLoadWh";
	modal_center_open('./WHM_POP_0007.clt', rtnary, 800,570,"no");
}

function rtnLoadWh(rtnVal){
	var formObj = document.frm1;
	if (rtnVal == "" || rtnVal == "undefined" || rtnVal == undefined) {
 		return;
	}else{
		...
		for (var i=0; i < rtnValAry.length; i++) {
			...
			docObjects[0].SetCellValue(idx, "cust_itm_id",itemArr[0],0);
			docObjects[0].SetCellValue(idx, "cb_itm_nm",itemArr[1],0);
			docObjects[0].SetCellValue(idx, "cb_ut_cd",itemArr[2],0);
			docObjects[0].SetCellValue(idx, "itm_inr_qty",itemArr[3],0);
			docObjects[0].SetCellValue(idx, "cb_loc_id",itemArr[4],0);
			docObjects[0].SetCellValue(idx, "itm_ctn_qty",itemArr[5],0);
			docObjects[0].SetCellValue(idx, "itm_total_qty",itemArr[7],0);
			
			docObjects[0].SetCellValue(idx, "itm_dim_wgt",itemArr[8],0);
			docObjects[0].SetCellValue(idx, "itm_dim_wgt_lbs",itemArr[9],0);
			
			docObjects[0].SetCellValue(idx, "itm_act_wgt",itemArr[10],0);
//			docObjects[0].SetCellValue(idx, "itm_act_wgt_lbs",roundXL(itemArr[9] * CNVT_CNST_KG_LB, 2),0);
			
			docObjects[0].SetCellValue(idx, "itm_vol",itemArr[11],0);
			docObjects[0].SetCellValue(idx, "itm_vol_cft",itemArr[12],0);

			docObjects[0].SetCellValue(idx, "itm_len",itemArr[13],0);
			docObjects[0].SetCellValue(idx, "itm_wdt",itemArr[14],0);
			docObjects[0].SetCellValue(idx, "itm_hgt",itemArr[15],0);
			docObjects[0].SetCellValue(idx, "item_sys_no",itemArr[16],0);
			
			docObjects[0].SetCellValue(idx, "itm_ctn_qty",0,1);
