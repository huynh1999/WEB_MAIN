function doWork(srcName){
    ...
    switch(srcName) {
		case "SEARCHLIST":
			...
			sheetObj.DoSearch("MenuMngSub_popupGS.clt", FormQueryString(formObj) );
		break;
            ...
            //if(inpuValCheck(sheetObj, MODIFY)){
                //if(confirm(getLabel('FMS_COM_CFMMOD'))){
                  ...
                  //console.log(FormQueryString(formObj));
                    sheetObj.DoSave("MenuMngSub_popupGS.clt", FormQueryString(formObj),"ibflag",false);
                //}
           // }
...
function sheet1_OnClick(sheetObj, row, col){
	   ...
	   var curMnuSeq=sheetObj.GetCellValue(row, "mnu_seq");
	   frm1.f_cur_mnu_seq.value=curMnuSeq;
	   //alert(frm1.f_cur_mnu_seq.value);
	}
//--------------------------------------------------------------------------------------------------------------
//IBSheet 설정
//--------------------------------------------------------------------------------------------------------------
...
function initSheet(sheetObj,sheetNo) {
	switch(sheetNo) {
		...
		with (sheetObj) {
	        ...
	        var headers = [ { Text:getLabel('MNU_SUB_POPUP_HDR'), Align:"Center"} ];
	        ...
	        var cols = [ 
	               ...
	               {Type:"Radio",     Hidden:0, Width:60,   Align:"Center",  ColMerge:1,   SaveName:"rdo_btn",      KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1 , TrueValue:"Y" ,FalseValue:"N" },
	               {Type:"Text",      Hidden:0,  Width:80,   Align:"Center",  ColMerge:1,   SaveName:"mnu_nm",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0 },
	               {Type:"Text",      Hidden:1,  Width:80,   Align:"Center",  ColMerge:1,   SaveName:"prnt_mnu_seq",   KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0 },
...
function sheet1_OnSearchEnd(sheetObj, errMsg){
	var formObj=document.frm1;
	//alert(formObj.f_prnt_mnu_seq.value);
	
	//~~~~~~~~~ 2016.12.14 일 추가 시작 원래 상위 모듈 보여주기 ~~~~~~~~~~~~~~~~~~~~~~~~
	for(var i=1 ; i < sheetObj.LastRow() + 1 ; i++){
		if(sheetObj.GetCellValue(i , "mnu_seq") == frm1.f_prnt_mnu_seq.value ){
			//sheetObj.SetCellValue(i , "rdo_btn", 1);
			//sheetObj.DeleteRow(i);
			//sheetObj.deletedItems(i);
